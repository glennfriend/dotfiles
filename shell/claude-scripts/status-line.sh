...
# Get user's last message and show on a second line
if [[ -n "$transcript_path" && -f "$transcript_path" ]]; then
    last_user_msg=$(jq -rs '
    ...
    if [[ -n "$last_user_msg" ]]; then
        # Get terminal width, cap message to fit
        term_width=$(tput cols 2>/dev/null || echo 120)
        max_msg=$((term_width - 4))
        if [[ ${#last_user_msg} -gt $max_msg ]]; then
            echo "💬 ${last_user_msg:0:$((max_msg - 3))}..."
        else
            echo "💬 ${last_user_msg}"
        fi
    fi
