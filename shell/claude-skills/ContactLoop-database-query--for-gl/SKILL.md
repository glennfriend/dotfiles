...
我們有 CL1 produciton 重要環境, 客戶比較少, 資料量大, 有些 table 筆數很多, 要注意效能
...
```bash
sql="
SELECT id,name,email FROM users WHERE id < 100 LIMIT 1
"
onr-samurai-local-db "${sql}"         # for localhost
onr-samurai-cl1-sshdb "${sql}"        # for CL1 production
...
onr-ava-cl1-sshdb "${sql}"            # for CL1 production
